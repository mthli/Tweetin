package io.github.mthli.Tweetin.Unit;

public class Flag {
    public static final int MAIN_TASK_DIED = 0x100;
    public static final int MAIN_TASK_ALIVE = 0x101;

    public static final int TWEET_STATUS_NONE = 0x200;
    public static final int TWEET_STATUS_BY_ME = 0x201;

    public static final int POST_ID = 0x300;
    public static final int POST_ORIGINAL = 0x301;
    public static final int POST_REPLY = 0x302;
    public static final int POST_RETWEET_QUOTE = 0x303;
    public static final int POST_SELECT_PICTURE = 0x304;

    public static final int MENTION_TASK_DIED = 0x400;
    public static final int MENTION_TASK_ALIVE = 0x401;

    /* Do something */
}
