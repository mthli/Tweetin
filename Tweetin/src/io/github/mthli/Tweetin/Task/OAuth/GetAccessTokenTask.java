...
public class GetAccessTokenTask extends AsyncTask<Void, Void, Boolean> {
    ...
    public GetAccessTokenTask(MainActivity mainActivity, String oAuthVerifier) {
    ...
    protected void onPreExecute() {
        ...
        progressDialog.setMessage(mainActivity.getString(R.string.main_pd_get_access_token));
        progressDialog.setCancelable(false);
        progressDialog.show();

        sharedPreferences = mainActivity.getSharedPreferences(mainActivity.getString(R.string.sp_tweetin), Context.MODE_PRIVATE);
    ...
    protected void onPostExecute(Boolean result) {
        if (result) {
            editor.putString(mainActivity.getString(R.string.sp_use_screen_name), useScreenName);
            editor.putString(mainActivity.getString(R.string.sp_access_token), accessToken.getToken());
            editor.putString(mainActivity.getString(R.string.sp_access_token_secret), accessToken.getTokenSecret());
            editor.commit();
            ...
            Toast.makeText(mainActivity, mainActivity.getString(R.string.main_toast_initializing), Toast.LENGTH_SHORT).show();
        } else {
            editor.putString(mainActivity.getString(R.string.sp_request_token), null);
            editor.putString(mainActivity.getString(R.string.sp_request_token_secret), null);
            editor.commit();

            progressDialog.hide();
            progressDialog.dismiss();

            Toast.makeText(mainActivity, mainActivity.getString(R.string.main_toast_get_access_token_failed), Toast.LENGTH_LONG).show();
        }
    }
}
